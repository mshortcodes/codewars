...
string would be aaabbbbhaijjjm meaning that the printer used three times color a, 
...
as a string representing a rational whose numerator is the number of errors and the 
...
function printerError(s) {
	...
	for (let letter of s) {
		if (letters.bad.includes(letter)) {
			errors += 1;
		}
	}

	const numerator = String(errors);
	const denominator = String(s.length);

	const result = `${numerator}/${denominator}`;
	return result;
}

console.log(printerError('aaaalopezzzzmkxxxy'));
